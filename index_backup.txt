...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Emoji-only messages — no bubble container, just big emoji */
...
<body tabindex="-1">
  ...
  <script>
    ...
    // Returns true if the entire string consists of only emoji characters
    // (and optional whitespace between them). Uses Unicode emoji ranges.
    function isEmojiOnly(str) {
      if (!str || !str.trim()) return false;
      // Strip all whitespace and check that what remains is only emoji codepoints
      const stripped = str.replace(/\s/g, '');
      if (!stripped) return false;
      // Regex: matches sequences of emoji characters (including ZWJ sequences,
      // variation selectors, regional indicators, keycap sequences, etc.)
      const emojiRegex = /^(?:\p{Emoji_Presentation}|\p{Extended_Pictographic}|\p{Emoji}\uFE0F|\uD83C[\uDDE0-\uDDFF][\uD83C][\uDDE0-\uDDFF]|[\u0023\u002A\u0030-\u0039]\uFE0F?\u20E3|\u200D)+$/u;
      return emojiRegex.test(stripped);
    }

    // Apply emoji-only class to all rendered message bubbles in the chat
    ...
    // Note: we also add a "stable" key helper that excludes reactions so we can
    // detect reaction-only updates and avoid re-rendering the entire chat.
    function getMessageStableKey(el) {
        ...
        if (this.status === 200) {
          ...
          if (newMessages.length > currentMessages.length) {
            ...
            if (prefixKeys === curKeys.join('~~')) {
              ...
              // Preserve visual scroll position when removing temporary indicators
              ...
              if (!isUploading()) {
                document.querySelectorAll('[data-upload-uid]').forEach(el => el.remove());
              }
              // If still uploading, indicators stay pinned where they are — don't move them

              // Adjust scrollTop by the change in scrollHeight so the viewport
              // doesn't visibly jump when elements are removed.
              const newScrollHeight = chatBox.scrollHeight;
              const delta = newScrollHeight - prevScrollHeight;
              chatBox.scrollTop = Math.max(0, prevScrollTop + delta);

              if (isFirstLoad) {
                isFirstLoad = false;
                scrollToBottom(true, true); // instant on first load
              } else if (wasAtBottom || shouldAutoScroll) {
                // Use RAF to schedule a smooth scroll once layout has settled.
                requestAnimationFrame(() => requestAnimationFrame(() => scrollToBottom(true, false)));
              } else {
            ...
            // Handle reaction-only updates without full re-render.
            // If the number of messages is the same and every message's stable key
            // (sender|time|content) matches, but the reactions differ, update only
            // the `.msg-reactions` container in-place to avoid layout jumps.
            if (newMessages.length === currentMessages.length) {
              let onlyReactionsChanged = true;
              for (let i = 0; i < newMessages.length; i++) {
                if (getMessageStableKey(newMessages[i]) !== getMessageStableKey(currentMessages[i])) {
                  onlyReactionsChanged = false;
                  break;
                }
              }
              if (onlyReactionsChanged) {
                for (let i = 0; i < newMessages.length; i++) {
                  ...
                  const neu = newMessages[i];
                  const newReacts = neu.querySelector('.msg-reactions');
                  const curReacts = cur.querySelector('.msg-reactions');
                  if (newReacts && curReacts) {
                    curReacts.innerHTML = newReacts.innerHTML;
                  } else if (newReacts && !curReacts) {
                    const wrapper = cur.querySelector('.bubble-wrapper');
                    const clone = newReacts.cloneNode(true);
                    if (wrapper) wrapper.appendChild(clone);
                    else cur.appendChild(clone);
                  } else if (!newReacts && curReacts) {
                    curReacts.remove();
                  }
                }
          ...
          // Preserve scroll position where possible to avoid visual jumps.
          const prevScrollTopFull = chatBox.scrollTop;
          const prevScrollHeightFull = chatBox.scrollHeight;
          ...
          const newScrollHeightFull = chatBox.scrollHeight;
          const deltaFull = newScrollHeightFull - prevScrollHeightFull;
          chatBox.scrollTop = Math.max(0, prevScrollTopFull + deltaFull);
          ...
          if (currentMessageCount > 0 && (wasAtBottom || shouldAutoScroll || isFirstLoad)) {
            ...
            isFirstLoad = false;
            // Use RAF for smooth scroll after re-render, instant on first load.
    ...
    (function restoreUploadIndicators() {
      ...
      saved.forEach(function({ uid, label, initials }) {
        ...
        `;
        // Append restored upload indicator to floating overlay so it doesn't reflow chat
        const overlay1 = getSendingOverlay();
        if (overlay1) overlay1.appendChild(ind);
        else chatBox.appendChild(ind);
        uploadingCount++;
        shouldAutoScroll = true;
        scrollToBottom(true);
      });
      ...
      const stalledInterval = setInterval(function() {
        ...
        if (!document.querySelector('[data-upload-uid]')) {
    ...
    function uploadFiles(fileList) {
      ...
      uploadIndicator.innerHTML = `
        ...
        <div class="message-avatar">${initials}</div>
      `;
      // Append upload indicator into the floating overlay so it doesn't disrupt chat flow
      const overlay2 = getSendingOverlay();
      if (overlay2) overlay2.appendChild(uploadIndicator);
      else chatBox.appendChild(uploadIndicator);
      shouldAutoScroll = true;
      userScrolledUp = false;
