...
function getCombinedChatAndUploads($chatlogFile, $uploadsDir, $currentUser) {
    ...
    if (empty($messages)) {
        $html = "<div class='empty-chat'>
                    ...
                    <h3>Welcome to Twitter by Finnapple</h3>
                    <p>Twitter (Clone) is a versatile, modern real-time chat application built with PHP and vanilla JavaScript that works ANYWHERE - whether you're on a local area network (LAN) or browsing from anywhere in the world! No complicated setup, no database required - just pure, instant communication.</p>
                </div>";
    }
